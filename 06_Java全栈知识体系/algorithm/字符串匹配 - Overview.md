...
![](https://www.pdai.tech/images/alg/alg-domain-char-match-11.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-match-12.png)

上图描述了常见字符串匹配算法的预处理和匹配时间。

![](https://www.pdai.tech/images/alg/alg-domain-char-1.gif)
...
- [朴素的字符串匹配算法（Naive String Matching Algorithm)](字符串匹配 - 模式预处理：朴素算法（Naive)(暴力破解).md)
  - 朴素的字符串匹配算法又称为**暴力匹配算法**（Brute Force Algorithm），最为简单的字符串匹配算法
- [Knuth-Morris-Pratt 字符串匹配算法（即 KMP 算法）](字符串匹配 - 模式预处理：KMP 算法（Knuth-Morris-Pratt）.md)
  - Knuth-Morris-Pratt算法（简称KMP）是最常用的字符串匹配算法之一
- [Boyer-Moore 字符串匹配算法](字符串匹配 - 模式预处理：BM 算法 (Boyer-Moore).md)
  - 各种文本编辑器的"查找"功能（Ctrl+F），大多采用Boyer-Moore算法，效率非常高
- [字符串匹配 - 文本预处理：后缀树（Suffix Tree）](字符串匹配 - 文本预处理：后缀树（Suffix Tree）.md)

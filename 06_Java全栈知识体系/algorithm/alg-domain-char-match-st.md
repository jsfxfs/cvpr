...
> 上述字符串匹配算法(朴素的字符串匹配算法, KMP 算法, Boyer-Moore算法)均是通过对**模式（Pattern）字符串进行预处理**的方式来加快搜索速度。对 Pattern 进行预处理的最优复杂度为 O(m)，其中 m 为 Pattern 字符串的长度。那么，有没有对文本（Text）进行预处理的算法呢？本文即将介绍一种**对 Text 进行预处理**的字符串匹配算法：后缀树（Suffix Tree）。@pdai
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-1.png)

在《[字典树(前缀树)](alg-basic-tree-trie.md)》一文中，介绍了一种特殊的树状信息检索数据结构：字典树（Trie）。Trie 将关键词中的字符按顺序添加到树中的节点上，这样从根节点开始遍历，就可以确定指定的关键词是否存在于 Trie 中。

下面是根据集合 {bear, bell, bid, bull, buy, sell, stock, stop} 所构建的 Trie 树。

![](https://www.pdai.tech/images/alg/alg-domain-char-st-2.png)

我们观察上面这颗 Trie，对于关键词 "bear"，字符 "a" 和 "r" 所在的节点没有其他子节点，所以可以考虑将这两个节点合并，如下图所示。

![](https://www.pdai.tech/images/alg/alg-domain-char-st-3.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-4.png)

然后，将独立的节点合并，形成 Compressed Trie。

![](https://www.pdai.tech/images/alg/alg-domain-char-st-5.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-6.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-7.jpg)

在 1995 年，Esko Ukkonen 发表了论文《[On-line construction of suffix trees](https://www.cs.helsinki.fi/u/ukkonen/SuffixT1withFigs.pdf)》，描述了在线性时间内构建后缀树的方法。下面尝试描述 Ukkonen 算法的基本实现原理，从简单的字符串开始描述，然后扩展到更复杂的情形。
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-8.png)

其代表的后缀意义如下。

![](https://www.pdai.tech/images/alg/alg-domain-char-st-9.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-10.png)

其代表的后缀意义如下。

![](https://www.pdai.tech/images/alg/alg-domain-char-st-11.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-12.png)

其代表的后缀意义如下。

![](https://www.pdai.tech/images/alg/alg-domain-char-st-13.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-14.png)

当 "#" 继续向后挪动一位，即第 4 位时，隐含地意味着已有的边会自动的扩展为：

![](https://www.pdai.tech/images/alg/alg-domain-char-st-15.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-16.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-17.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-18.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-19.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-20.png)

继续上面的操作，插入最终后缀 "x"。因为活动点中的 active_length 已经降到 0，所以插入操作将发生在 root 上。由于没有以 "x" 为前缀的边，所以插入一条新的边：

![](https://www.pdai.tech/images/alg/alg-domain-char-st-21.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-22.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-23.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-24.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-25.png)

由于此处又创建了一个新的内部节点，依据 Rule 2，我们需要建立一条与前一个被创建内节点的后缀连接。

![](https://www.pdai.tech/images/alg/alg-domain-char-st-26.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-27.png)
...
![](https://www.pdai.tech/images/alg/alg-domain-char-st-28.png)

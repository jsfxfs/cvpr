...
https://www.elastic.co/guide/cn/elasticsearch/guide/current/search-in-depth.html 里面提供了多维度、更灵活的搜索场景以及案例。
...
- 多级排序
  ...
  比如我们想要结合使用 date 和 _score 进行查询，并且匹配的结果首先按照日期排序，然后按照相关性排序。https://www.elastic.co/guide/cn/elasticsearch/guide/current/_Sorting.html 。多级排序并不一定包含 _score 。你可以根据一些不同的字段进行排序， 如地理距离或是脚本计算的特定值。

...
> 我们在前文已经阐述了[Redis 5种基础数据类型详解](Redis入门 - 数据类型：5种基础数据类型详解.md)，分别是字符串(string)、列表(list)、哈希(hash)、集合(set)、有序集合(zset)；那么这些基础类型的底层是如何实现的呢？Redis的每种对象其实都由**对象结构(redisObject)** 与 **对应编码的数据结构**组合而成, 前文是第一部分[对象机制详解](Redis进阶 - 数据结构：对象机制详解.md), 本文主要介绍**底层数据结构** 部分。（在这特别提下，大多数人构建知识体系去读源码是不太现实的，这时候我认为更为重要的是，理解为何会出现这样的设计，知道它解决了什么问题，然后对应了解相关知识点，最终在一些应用场景时可以以此来通过调整存储的类型进一步提升优化性能）。@pdai
...
![](https://www.pdai.tech/images/db/redis/redis-ds-2.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-5.png)

整个ziplist在内存中的存储格式如下：

![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-6.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-7.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-4.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-13.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-8.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-9.png)

如果我们增加如下两级索引，那么它搜索次数就变成了3次

![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-10.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-ds-x-11.png)

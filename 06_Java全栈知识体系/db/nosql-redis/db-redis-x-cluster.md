...
> 前面两篇文章，[主从复制](db-redis-x-copy.md)和[哨兵机制](db-redis-x-sentinel.md)保障了高可用，就读写分离而言虽然slave节点扩展了主从的读并发能力，但是**写能力**和**存储能力**是无法进行扩展，就只能是master节点能够承载的上限。如果面对海量数据那么必然需要构建master（主节点分片)之间的集群，同时必然需要吸收高可用（主从复制和哨兵机制）能力，即每个master分片节点还需要有slave节点，这是分布式系统中典型的纵向扩展（集群的分片技术）的体现；所以在Redis 3.0版本中对应的设计就是Redis Cluster。@pdai
...
![](https://www.pdai.tech/images/db/redis/redis-cluster-3.png)
...
![](https://www.pdai.tech/images/db/redis/redis-cluster-4.png)

### ASK 重定向

Ask重定向发生于集群伸缩时，集群伸缩会导致槽迁移，当我们去源节点访问时，此时数据已经可能已经迁移到了目标节点，使用Ask重定向来解决此种情况。

![](https://www.pdai.tech/images/db/redis/redis-cluster-5.png)
...
![](https://www.pdai.tech/images/db/redis/redis-cluster-6.png)

- 将上述映射关系存到本地，通过映射关系就可以直接对目标节点进行操作（CRC16(key) -> slot -> node），很好地避免了Moved重定向，并为每个节点创建JedisPool
- 至此就可以用来进行命令操作

![](https://www.pdai.tech/images/db/redis/redis-cluster-7.png)
...
![](https://www.pdai.tech/images/db/redis/redis-cluster-1.png)
...
![](https://www.pdai.tech/images/db/redis/redis-cluster-2.png)
...
我们知道一致性hash算法是2的16次方，为什么hash slot是2的14次方呢？作者[原始回答](https://github.com/redis/redis/issues/2576)
...
![](https://www.pdai.tech/images/db/redis/db-redis-cluster-1.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-cluster-2.png)
...
![](https://www.pdai.tech/images/db/redis/db-redis-cluster-3.png)

...
> 在学习完[底层数据结构](Redis进阶 - 数据结构：底层数据结构详解.md)之后，我们终于可以结合前文内容阐述redis对象及编码之间的关系了。@pdai

## redis对象与编码(底层结构)对应关系引入

在对[对象机制详解](Redis进阶 - 数据结构：对象机制详解.md) 和 [底层数据结构](Redis进阶 - 数据结构：底层数据结构详解.md) 有了初步认识之后，我们便可以继续理解它们是怎么对应的：

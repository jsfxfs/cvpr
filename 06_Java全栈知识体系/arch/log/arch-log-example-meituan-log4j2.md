...
> 日志导致线程Block的问题，相信你或许已经遇到过，对此应该深有体会；或许你还没遇到过，但不代表没有问题，只是可能还没有触发而已。关于日志框架（日志门面）建议先理解下这篇[常用开发库 - 日志类库详解](../../develop/package/dev-package-x-log.md)。@pdai
...
![](https://www.pdai.tech/images/arch/log/log-log4j-1.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-2.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-2-2.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-3.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-6.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-7.png)

从Blocked线程堆栈不难看出是跟日志打印相关，由于是ERROR级别日志，查看具体报错日志，发现有两种业务异常，分别如下图8和图9所示：

![](https://www.pdai.tech/images/arch/log/log-log4j-8.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-9.png)

这些业务异常会是导致线程Block的幕后元凶吗？接下来本章节将结合如下图10所示的调用链路深入分析线程Block的根因。

![](https://www.pdai.tech/images/arch/log/log-log4j-10.png)

#### 3.2.2 为什么会Block线程？

从Blocked线程堆栈中可以看出，线程阻塞在类加载流程上，查看WebAppClassLoader相关代码片段如下图11所示，发现加载类时确实会根据类名来加synchronized同步块，因此初步猜测是类加载导致线程Block。

![](https://www.pdai.tech/images/arch/log/log-log4j-11.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-12.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-13.png)
...
- 默认使用native方法进行反射操作。
- 一定条件下会生成字节码进行反射操作，即生成`sun.reflect.GeneratedMethodAccessor<N>`类，它是一个反射调用方法的包装类，代理不同的方法，类后缀序号递增。
...
![](https://www.pdai.tech/images/arch/log/log-log4j-14.png)
...
从`MethodAccessorGenerator#generateName`方法可以看到，字节码生成的类名称规则是`sun.reflect.GeneratedConstructorAccessor<N>`，其中N是从0开始的递增数字，且生成类是由DelegatingClassLoader类加载器定义，所以其他类加载器无法加载该类，也就无法生成类缓存数据，从而导致每次加载类时都需要遍历JarFile，极大地降低了类查找速度，且类加载过程是synchronized同步调用，在高并发情况下会更加恶化，从而导致线程Block。
...
![](https://www.pdai.tech/images/arch/log/log-log4j-15.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-16.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-16-2.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-17.png)

从Blocked线程堆栈不难看出是和日志打印相关，由于是ERROR级别日志，查看具体报错日志，发现如下图18所示的业务异常。

![](https://www.pdai.tech/images/arch/log/log-log4j-18.png)

本案例的Blocked线程堆栈和上述“AsyncAppender导致线程Block”案例一样，那么导致线程Block的罪魁祸首会是业务异常吗？接下来本章节将结合下图19所示的调用链路深入分析线程Block的根因。

![](https://www.pdai.tech/images/arch/log/log-log4j-19.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-20.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-21.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-22.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-22-2.png)

![](https://www.pdai.tech/images/arch/log/log-log4j-23.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-24.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-25.png)
...
![](https://www.pdai.tech/images/arch/log/log-log4j-26.png)
...
- 作者：志洋、陈超、李敏、凯晖、殷琦等，均来自美团基础技术部-应用中间件团队。
- 版权声明：本文为「美团技术团队」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
- 原文链接：https://tech.meituan.com/2022/07/29/tips-for-avoiding-log-blocking-threads.html
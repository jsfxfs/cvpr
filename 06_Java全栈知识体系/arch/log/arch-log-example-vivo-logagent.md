...
![](https://www.pdai.tech/images/arch/log/log-agent-1.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-2.png)
...
- 具备断点续传能力，升级和停止不丢数据
- 具备可视化的、中心化的采集任务管理平台
- 丰富的监控指标与告警（包括采集流量、时效性、完整性等）
- 低系统资源开销（包括磁盘、内存、CPU及网络等）
...
![](https://www.pdai.tech/images/arch/log/log-agent-3.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-4.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-5.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-6.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-7.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-8.png)
...
> Agent采集进程是随着业务进程一起部署在一个机器上的，共同使用业务机器的资源（CPU、内存、磁盘、网络），所以在设计时，要考虑控制好Agent采集进程对机器资源的消耗，同时要做好对Agent进程对机器资源消耗的监控。**一方面保障业务有稳定的资源可以正常运行；另外可以保障Agent自身进程正常运作**。
...
![](https://www.pdai.tech/images/arch/log/log-agent-9.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-10.png)
...
![](https://www.pdai.tech/images/arch/log/log-agent-11.png)

## 六、总结

前文介绍了vivo日志采集Agent在设计过程中的一些核心技术点：包括**日志文件的发现与监听、日志文件的唯一标识符设计、日志文件的实时采集与离线采集的架构设计、日志文件的清理策略、采集进程对系统资源的消耗控制、平台化管理的思路**等，这些关键的设计思路覆盖了自研采集agent大部分的核心功能，同时也覆盖了其中的难点痛点，能让后续的开发环节更加畅通。当然，还有一些高阶的采集能力未涵盖本文介绍在内，比如"如何做好日志采集数据的完整性对账"，"数据库类型的场景的采集设计"等，大家可以继续探索解决方案。
...
- 原文链接：https://mp.weixin.qq.com/s/Wya9TLEZpVkypiAmyZ_V_Q
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-1.png)

在微服务和 Serverless 的喧嚣中，也逐渐出现了很多质疑和反对的声音：越来越多的人发现，当他们兴冲冲的迁移单体应用到微服务和 Serverless 架构之后，得到的收益并没有期望中的那么理想。最近，出现了对微服务的各种质疑、反思的声音，甚至放弃微服务回归单体。
...
问题源于微服务实施的方式 —— 按照业务逻辑拆解单体，划分为多个微服务，定义 API 接口，然后通过 REST 或者 RPC 进行远程调用，最终把这些微服务组合起来提供各种业务功能。简单说，就是在业务拆分的基础上，用进程间的**远程调用**简单替代原来**进程内的方法调用**。期间，对于原来使用的各种分布式能力，继续沿用之前的方式，简单说：方式不变，只是粒度变小。
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-2.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-3.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-4.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-5.png)

从单体到微服务，直接方法调用被替换为远程调用（REST 或者 RPC），即使采用 Service Mesh 也只是在链路中多增加了 Sidecar 节点，并未改变远程调用的性质：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-6.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-7.png)
...
> 关于这一点，在 "[The Eight Fallacies of Distributed Computing/分布式计算的8个谬论](../arch-z-wrong.md)" 一文中有详细阐述。

类似的，在采用 Function 时，如果依然沿用上面的方式，以单体或微服务架构的思维方式和设计模式来创建 FaaS/Serverless 架构：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-8.png)

其本质不会发生变化 —— 不过是将微服务变成粒度更小的函数，导致系统中的远程调用数量大为增加：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-12.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-9.png)

最近出现的 Multiple Runtime / Mecha 架构，以及遵循这一架构思想的微软开源产品 Dapr ，则将这个做法推进到服务间通讯之外更多的分布式能力。

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-10.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-11.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-13.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-14.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-15.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-16.png)

真实业务场景下各个组件（微服务或者 Function）的业务逻辑关系，通常不会像上图这么夸张，不应该全是 Command （后面会谈到也不应该全是 Event） ，而应该是类似下图描述的两者结合，以微服务为例（Function 类推）：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-17.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-18.png)

将图中涉及的微服务 A/B/C/D/E 在处理业务逻辑的行为总结下来，通讯行为大体是一样的：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-19.png)

抽象起来，一个典型的微服务在业务处理流程中的通讯行为可以概括为以下四点：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-20.png)
...
从业务逻辑处理的角度来看，外部访问的 Command 和内部操作的 Action 是业务逻辑的 “实现” 部分：这些操作组成了完整的业务逻辑——如果这些操作失败，则业务处理将会直接影响（失败或者部分失败）。而发布事件则是业务逻辑完成之后的后续 “通知” 部分：当业务逻辑处理完毕，状态变更完成后，以事件的方式驱动后续的进一步处理。注意是驱动，而不是直接操纵。 从时间线的角度来看整个业务处理流程如下图所示：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-21.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-22.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-23.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-24.png)
...
这里强烈推荐一篇博客文章， [Microservices Choreography vs Orchestration: The Benefits of Choreography](https://solace.com/blog/microservices-choreography-vs-orchestration/)，作者 Jonathan Schabowsky ，Solace 的 CTO。他在这边博客中总结了让微服务协同工作的两种模式，并做了一个生动的比喻：

- **编排（Orchestration）**：需要主动控制所有的元素和交互，就像指挥家指挥乐团的乐手一样——对应 Command；
- **协调（Choreography）**：需要建立一个模式，微服务会跟随音乐起舞，不需要监督和指令——对应 Event；

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-25.png)

也曾看到很多持类似观点的文章，其中有一张图片印象深刻，我摘录过来：

![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-26.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-27.png)
...
- 而且还引入了一个非常麻烦的状态问题：即服务间通讯的上下文中通常是有状态的，Reply Event 必须准确的发送给 Request Event 的发起者的实例，而不能任意选择一个。这使得 Reply Event 不仅仅要 1:1 的绑定订阅者服务，还必须绑定这个服务的特定实例 —— 这样的 Reply Event 已经没法称为 Event 了；
  ...
  - ![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-28.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-29.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-30.png)
...
![](https://www.pdai.tech/images/arch/microservice/arch-m-singleton-31.png)
...
- [The Eight Fallacies of Distributed Computing](https://www.red-gate.com/simple-talk/blogs/the-eight-fallacies-of-distributed-computing/): 分布式计算领域的经典文章，中文翻译请见 分布式计算的八大谬论
...
- [Building Event-Driven Cloud Applications and Services](https://medium.com/@ratrosy/building-event-driven-cloud-applications-and-services-ad0b5b970036): 讨论构建事件驱动的应用和服务的通用实践和技术，是一个序列教程。中文翻译看 构建事件驱动的云应用和服务
...
- [致传统企业朋友：不够痛就别微服务](https://www.infoq.cn/article/Nd0RofAUp0WtlvlQArbu)，有坑：网易云刘超刘老师的超级好文章，极其实在而全面的讲述微服务落地需要考虑的方方面面以及各种问题，强烈推荐阅读。

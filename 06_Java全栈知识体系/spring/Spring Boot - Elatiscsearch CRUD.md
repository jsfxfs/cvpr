...
- 常规来说，采用spring-boot-starter-data-elasticsearch会简单很多，但是在有些动态的复合查询时，或者结合多index动态查询时，需要考虑一定的业务二次封装；可以参考我在[springboot多数据源](Spring Boot - 多个数据源Service层封装.md)中针对ElasticSearch封装代码。

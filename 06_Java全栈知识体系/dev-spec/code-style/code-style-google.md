...
> Google 出品的 《[Google Java Code Style](https://google.github.io/styleguide/javaguide.html)》，由[fantasticmao](https://github.com/fantasticmao)翻译，对应[github仓库](https://github.com/fantasticmao/google-java-style-guide-zh_cn)。@pdai
...
1. *class* 类 表示 *ordinary class* 普通的类、*enum class* 枚举类、*interface* 接口或 *annotation* 注解类型。
2. *member* 成员 表示 *nested class* 嵌套类、*field* 字段、*method* 方法或 *constructor* 者构造方法，即除初始化方法和注释之外，类的所有最顶层内容。
3. *comment* 注释 表示 *implementation comments* 实现注释。我们不使用术语 *documentation comments*，而是使用（在 Java 中）更通用的术语 *Javadoc*。
...
源文件的名称包含了区分大小写的（并且是 [唯一](#%E6%9C%89%E4%B8%94%E4%BB%85%E6%9C%89%E4%B8%80%E4%B8%AA%E9%A1%B6%E7%BA%A7%E7%B1%BB%E7%9A%84%E5%A3%B0%E6%98%8E) 的）顶级类的类名和 `.java` 扩展名组成。
...
Package 语句不允许换行。单行字符限制（ [列限制：100](#%E5%88%97%E9%99%90%E5%88%B6100) 章节）不适用于 Package 语句。
...
Import 语句 **不允许** 换行。单行字符限制（ [列限制：100](#%E5%88%97%E9%99%90%E5%88%B6100) 章节）不适用于 Import 语句。
...
**术语说明**：*block-like construct* 块状结构 指类或者普通方法或者构造方法的主体。注意，在后续 [数组初始化](#%E6%95%B0%E7%BB%84%E5%88%9D%E5%A7%8B%E5%8C%96%E5%8F%AF%E4%BB%A5%E5%86%99%E6%88%90%E5%9D%97%E7%8A%B6%E7%BB%93%E6%9E%84) 章节中，任何数组的初始化可以选择被认为是一个块状结构。
...
关于枚举类的一些特殊情况，将在 [枚举类](#%E6%9E%9A%E4%B8%BE%E7%B1%BB) 章节说明。

#### 空语句块：可以简洁

一个空的语句块或者块状结构可以遵循 K & R 风格（正如在 [非空语句块](#%E9%9D%9E%E7%A9%BA%E8%AF%AD%E5%8F%A5%E5%9D%97k--r-%E9%A3%8E%E6%A0%BC) 章节所中描述的）。或者，当它不是 *multi-block statement* 多块语句（一个包含多块的语句，例如：`if / else`、`try / catch / finally` ）一部分的时候，可以在左花括号开始之后立即使用右花括号结束，`{}` 之中不包含任何字符或者换行符。
...
每当新写一个语句块或者块状结构时，增加 2 个空格的缩进。当语句块结束时，返回至上一级别的缩进。语句块的缩进规则适用于所有代码和注释。（代码示例请见 [非空语句块：K & R 风格](#%E9%9D%9E%E7%A9%BA%E8%AF%AD%E5%8F%A5%E5%9D%97k--r-%E9%A3%8E%E6%A0%BC) 章节）
...
Java 代码的列限制为 100 个字符。这儿的「字符」意味着任意的 Unicode 码位。除非另有说明，任何超过此限制的代码行都必须被换行，正如在 [换行](#%E6%8D%A2%E8%A1%8C) 章节中所描述的。
...
- Package 语句和 Import 语句（请见 [Package 语句](#package-%E8%AF%AD%E5%8F%A5) 和 [Import 语句](#import-%E8%AF%AD%E5%8F%A5) 章节）。
- 注释中可能直接被复制粘贴执行的 Shell 命令。

### 换行

**术语说明**：将原本可以合法写在一行的代码拆分成多行，这种行为称作 *line-wrapping* 换行。
...
1. 当一行代码的中断发生在 *non-assignment* 非赋值运算符时，需要在该运算符之前换行。（注意这与其它语言的 Google 编程风格不同，例如 C++ 和 JavaScript）
...
2. 当一行代码的中断发生在 *assignment* 赋值运算符时，需要在该运算符之后换行，但在之前换行也可以接受。
...
在 [水平对齐](#%E6%B0%B4%E5%B9%B3%E5%AF%B9%E9%BD%90%E4%BB%8E%E4%B8%8D%E8%A6%81%E6%B1%82) 章节中介绍了不建议使用数量不确定的空格来与上一行代码中的某些单词（token）对齐。
...
1. 类中连续的成员或初始化方法之间，包括：字段、构造方法、方法、内部类、静态初始化代码块、实例初始化代码块。
   - **特殊情况**：两个连续字段（它们之间没有其它代码）之间的空行是可选的。可以根据实际需要，用空行去创建字段之间的 *logical groupings* 逻辑分组。
   - **特殊情况**：枚举常量之间的空行在 [枚举类](#%E6%9E%9A%E4%B8%BE%E7%B1%BB) 章节中介绍。
2. 本文档其它章节中所要求的（例如 [源文件结构](#%E6%BA%90%E6%96%87%E4%BB%B6%E7%BB%93%E6%9E%84) 章节和 [Import 语句](#import-%E8%AF%AD%E5%8F%A5) 章节）
...
**术语说明**：*Horizontal alignment* 水平对齐 是为了使某些单词（token）出现在上一行代码中的另一些单词的正下方，从而在代码中添加额外的数量不确定的空格的做法。
...
没有方法和注释的枚举常量可以写成数组初始化的方式（请见 [数组初始化](#%E6%95%B0%E7%BB%84%E5%88%9D%E5%A7%8B%E5%8C%96%E5%8F%AF%E4%BB%A5%E5%86%99%E6%88%90%E5%9D%97%E7%8A%B6%E7%BB%93%E6%9E%84) 章节）。
...
**术语说明**： switch 语句块的花括号内是一个或多个 *statement groups* 语句组。每个语句组都包含了一个或多个 switch 标签（ `case Foo:` 或者 `default:` ），switch 标签之后跟随着一条或多条语句（或者，对于最后一个语句组，它可以包含零条或多条语句）。
...
用于类、方法和构造方法的注解出现在文档之后，并且每个注解都写在它们所属的行上（这意味着，每个注解占用一行）。这些注解所占用的行不构成换行（请见 [换行](#%E6%8D%A2%E8%A1%8C) 章节），因此缩进级别不会增加。例如：
...
本章节介绍 *implementation comments* 实现注释。Javadoc 将在 [Javadoc](#javadoc) 章节单独介绍。
...
类名以 [大骆峰](#%E9%AA%86%E9%A9%BC%E5%B3%B0%E5%BD%A2%E5%BC%8F%E5%AE%9A%E4%B9%89) 方式编写。
...
方法名以 [小骆峰](#%E9%AA%86%E9%A9%BC%E5%B3%B0%E5%BD%A2%E5%BC%8F%E5%AE%9A%E4%B9%89) 方式编写。

方法名通常是动词或者动词短语。例如 `sendMessage` 或者 `stop`。

下划线可能为了分离命名上的逻辑组件，而出现在 Junit 测试方法名中，每个组件都以 [小骆峰](#%E9%AA%86%E9%A9%BC%E5%B3%B0%E5%BD%A2%E5%BC%8F%E5%AE%9A%E4%B9%89) 方式编写。一个典型的模式是 `<methodUnderTest>_<state>`，例如 `pop_emptyStack`。测试方法的命名没有一种唯一正确的方式。
...
非常量字段名（静态或者其它形式）以 [小骆峰](#%E9%AA%86%E9%A9%BC%E5%B3%B0%E5%BD%A2%E5%BC%8F%E5%AE%9A%E4%B9%89) 方式编写。

非常量字段名通常是名词或者名词短语。例如：`computedValues` 或者 `index`。

#### 参数名

参数名以 [小骆峰](#%E9%AA%86%E9%A9%BC%E5%B3%B0%E5%BD%A2%E5%BC%8F%E5%AE%9A%E4%B9%89) 方式编写。

public 方法中应该避免使用一个字符的参数名。

#### 局部变量名

局部变量名以 [小骆峰](#%E9%AA%86%E9%A9%BC%E5%B3%B0%E5%BD%A2%E5%BC%8F%E5%AE%9A%E4%B9%89) 方式编写。
...
- 以类名的方式命名（请见 [类名](#%E7%B1%BB%E5%90%8D) 章节），跟随着大写字母 T（例如：`RequestT`，`FooBarT` ）。
...
额外的 Javadoc 内容也可以出现，正如在 [非必需的 Javadoc](#%E9%9D%9E%E5%BF%85%E9%9C%80%E7%9A%84-javadoc) 章节中所描述的。
...
非必需的 Javadoc 内容不是严格要求遵守 [段落](#%E6%AE%B5%E8%90%BD) 章节、[块标签](#%E5%9D%97%E6%A0%87%E7%AD%BE) 章节以及 [摘要片段](#%E6%91%98%E8%A6%81%E7%89%87%E6%AE%B5) 章节的格式化规则，尽管这当然是推荐的。
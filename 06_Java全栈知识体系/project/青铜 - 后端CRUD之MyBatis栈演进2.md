...
> 本文是 **青铜 - 后端CRUD之MyBatis栈演进**的第二篇，上一篇是 [青铜 - 后端CRUD之MyBatis栈演进1](青铜 - 后端CRUD之MyBatis栈演进1.md)。@pdai
...
- 第二，过多的自定义的Util，而现有的项目项目中，Util一定不要自己造轮子，原则是优先使用[Guava](../develop/package/常用开发库 - Google Guava包.md)，[commons](../develop/package/常用开发库 - Apache Common包.md)等开源工具类；还有一些新的[hutool](../develop/package/常用开发库 - Hutool包.md)等工具库是可以满足绝大部分需求的，如果没有再添加。于此同时，如果项目规模稍微大一点和要求，还可以使用外观模式进行小量的封装，比如你同时使用了Guava，Common库中StringUtils，那么可以增加一个MyStringUtils，将常用方法融入这个类，对项目中所有类来说只使用MyStringUtils。此外这里的例子还有一些标注为@Component等模块作为util，造成了代码的耦合度增加。
...
关于JSON库可以参考 [常用类库 - JSON类库详解](../develop/package/常用开发库 - JSON库详解.md)：
...
PS:商业系统上线前都会做[渗透测试](../develop/security/开发安全实战 - 渗透测试流程示例.md)，有这些漏洞是无法上线的。

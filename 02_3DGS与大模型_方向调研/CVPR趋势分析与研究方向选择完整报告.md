...
| --- | ---: | --- | ---: | ---: | --- |
| CVPR 2026 | 4042 | 4042 / 缺 0 | 1803（44.6%） | 744（18.4%） | `05_CVPR数据与工具/cvpr_papers/` |
| CVPR 2025 | 2871 | 2871 / 缺 0 | 1245（43.4%） | 518（18.0%） | `05_CVPR数据与工具/cvpr_papers_2025/` |

- 爬取工具：`05_CVPR数据与工具/run_cvpr.py`（三段式流水线：元数据 → 摘要 → 分类导出），运行环境为 conda 环境 `scrapling`（Python 3.10.20 + bs4 4.14.3 + lxml + requests）。
- CVPR 2025 爬取命令（在 `05_CVPR数据与工具/` 目录下执行）：`conda run -n scrapling python run_cvpr.py --years 2025 --out cvpr_papers_2025`，2871 篇摘要 0 失败。
...
├── 01_游戏方向_无对局胜率评估/       # 并行主线：无对局数据的组合对抗胜率评估（与本报告独立）
├── 02_3DGS与大模型_方向调研/
│   ├── CVPR趋势分析与研究方向选择完整报告.md   # 本报告
│   ├── 方向选择_3DGS与大模型_最终分析.md       # v2.1 修订版结论
│   └── 定向统计/                   # niche_probe*.py + niche_probe*_results.json（三轮定向统计）
├── 03_MODAF调研/                   # MODAF 体系结构框架调研（课题组命题材料）
├── 04_文献_无人机集群效能/          # Quan CJA2025 论文 PDF + 逐页文本
└── 05_CVPR数据与工具/              # 爬虫与语料（本报告的数据底座）
    ...
    └── crawl.log                   # 最近一次全量爬取日志
```

> 注：`02_.../定向统计/` 下的脚本按自身位置自动定位语料（不再写死绝对路径），在任意目录直接 `python niche_probe3.py` 即可运行。

## 11.2 复现/扩展方法

> 以下命令均在 `05_CVPR数据与工具/` 目录下执行（`run_cvpr.py` 依赖同目录的 `cvpr/` 包与相对输出路径）。
